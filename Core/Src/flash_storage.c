 ...
 * @brief Implementation of flash memory storage API
 * @author Dennis Rathgeb
 * @date 2024
 *
 * Implements flash erase, read, write operations for STM32F030C8.
 * Uses HAL flash functions with proper unlock/lock sequence.
 */
 ...
 * This is the reflected form of the standard CRC32 polynomial.
 */
...
HAL_StatusTypeDef flash_erase_page(uint32_t page_addr)
{
    ...
    /* Verify address is within flash bounds */
    if (page_addr < FLASH_BASE || page_addr >= (FLASH_BASE + 0x10000)) {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef flash_write_data(uint32_t addr, const void* data, uint32_t size)
{
    ...
    /* Verify half-word alignment */
    if ((addr & 0x01) != 0) {
        return HAL_ERROR;
    }

    /* Verify address is within flash bounds */
    if (addr < FLASH_BASE || (addr + size) > (FLASH_BASE + 0x10000)) {
        return HAL_ERROR;
    }
    ...
    /* Write data in half-words (16-bit) */
    for (i = 0; i < size; i += 2) {
        uint16_t halfword;

        /* Handle last byte if size is odd */
        if (i + 1 < size) {
            halfword = bytes[i] | ((uint16_t)bytes[i + 1] << 8);
        } else {
            halfword = bytes[i] | 0xFF00;  /* Pad with 0xFF */
        }
...
HAL_StatusTypeDef flash_read_data(uint32_t addr, void* data, uint32_t size)
{
    ...
    /* Flash is memory-mapped, just copy */
